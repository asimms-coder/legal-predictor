...
def split_sents(t):
    return re.split(r"(?<=[\.\?\!])\s+(?=[A-Z0-9])", t.strip())

def drop_disposition_sents(sents):
    out = []
    for s in sents:
        if any(p.search(s) for p in DISP_PATTERNS):
            continue
        out.append(s)
    return out
...
def extract_label(case):
    candidates = [
        ...
        case.get("casebody", {}).get("disposition"),
    ]
...
def is_federal_appellate_or_scotus(case):
    ...
    return ("supreme court of the united states" in s or
            "u.s. court of appeals" in s or
            "court of appeals" in s or
            "federal circuit" in s or
            any(f"{i}th cir" in s or f"{i}th circuit" in s for i in range(1,13)))
            ...
            if fn.endswith(".jsonl"):
                ...
                with open(p, "r", encoding="utf-8") as f:
                    ...
                    if isinstance(obj, list):
                        for rec in obj: yield rec
                    elif isinstance(obj, dict) and "cases" in obj:
                        for rec in obj["cases"]: yield rec
                    else:
                        yield obj
...
def main(in_dir, out_csv, start_year, end_year):
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as out:
        ...
        for case in iter_cases(in_dir):
            if not is_federal_appellate_or_scotus(case): continue
            y = extract_label(case)
            if y not in {"AFFIRM","REVERSE","REMAND"}: continue
            yr = year_of(case)
            if not yr or yr < start_year or yr > end_year: continue
